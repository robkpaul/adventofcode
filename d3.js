...
class Wire {
  ...
  doManhattanEquation(coords1, coords2){

  }

  createPathFromVector(vector) {
    let axis = vector[0];
    let dist = vector[1];
    if(axis === 'x'){
      let startX = this.endX;
      this.endX += dist;
      if(dist < 0){
        for (let i = startX; i > endX; i--) {
          this.path[i][endY] = true;
        }
      }
      else {
        for (let i = startX; i < endX; i++) {
      ...
      let startY = this.endY;
      this.endY += dist;
      if(dist < 0){
        for (let i = startY; i > endY; i--) {
          this.path[endX][i] = false;
        }
      }
      else {
        for (let i = startY; i < endY; i++) {
  ...
  createPathFromVectorAndCompare(vector, otherWire){
    if(typeof path != "Wire"){
      throw `Unexpected type: Expected Wire, got ${typeof path}`;
    }
    let axis = vector[0];
    let dist = vector[1];
    if(axis === 'x'){
  ...
  parseVector(code) {
    let direction = code.charAt(0);
    let dist = parseInt(code.substring(1));
    let axis;
    ...
    if(direction === 'U' || direction === 'D'){
      ...
      throw ('The start of the string is not a valid direction!');
    }
    return [axis, dist];
  }
}
